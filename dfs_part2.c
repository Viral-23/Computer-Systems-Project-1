...
int findKeys(int arr[], int size, int keysFound, int shift) {
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        if (arr[i] == -1 && keysFound < H) {
            keysFound+=1;
            fprintf(f2, "Hi I'm process %d and I found the hidden key %d in position A[%d] \n", getpid(), keysFound, i + shift);
        }
    }
    fclose(f2);
    printf("\n");
...
int fileGenerator()
{
    ...
    fp = fopen ("results.txt", "w"); //clear results.txt for new results
    fclose(fp);
return 0;
}
...
*/
        ...
        sleep(100); //after child resumes: sleep for 100 sec
...
*/
        ...
        else if ( ) //child has hidden numbers >= H
        {
            signal(pid, SIGKILL);
        }
